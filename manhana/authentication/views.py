...
from manhana.core.models.parametro import VinculoServidorUnidade, UnidadeFluxoProcesso
import logging
...
class VerificaUsuarioView(LoginRequiredMixin, RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        if not user.pessoa.id_pessoa_sig:
            ...
            perfis_servidor = ServidorProfile.objects.filter(pessoa=user.pessoa, is_ativo=True)
            perfis_discente = DiscenteProfile.objects.filter(pessoa=user.pessoa, is_ativo=True)
            ...
            if perfis_servidor.count() == 1:
                if perfis_discente.count() == 0:
                    try:
                        ...
                        # if servidor.categoria == 'Docente':
                        #     docente = DocenteProfile.objects.get(pk=servidor.pk)
                        #     # info_sigs.carregar_informacoes_funcionais(user, perfil=docente)
                        # else:
                        #     # info_sigs.carregar_informacoes_funcionais(user, perfil=servidor)
...
class InfoAdicionaiUserView(LoginRequiredMixin, generic.FormView):
        ...
        with transaction.atomic():
            ...
            if pessoas['meta']['total_count'] == 1:
                for p in pessoas['objects']:
                    ...
                    perfis_docente = DocenteProfile.objects.filter(pessoa=user.pessoa, is_ativo=True)
                    perfis_discente = DiscenteProfile.objects.filter(pessoa=user.pessoa, is_ativo=True)
                    perfis_tae = TaeProfile.objects.filter(pessoa=user.pessoa, is_ativo=True)
    ...
    def get_context_data(self, **kwargs):
        context = super(InfoAdicionaiUserView, self).get_context_data(**kwargs)
        context['progress'] = 10
...
class SelecionarPerfilView(LoginRequiredMixin, generic.View):
    ...
    def carregar_context(self, **kwargs):
        ...
        self.context['perfis_docente'] = DocenteProfile.objects.filter(pessoa=user.pessoa, is_ativo=True)
        self.context['perfis_discente'] = DiscenteProfile.objects.filter(pessoa=user.pessoa, is_ativo=True)
        self.context['perfis_tae'] = TaeProfile.objects.filter(pessoa=user.pessoa, is_ativo=True)
